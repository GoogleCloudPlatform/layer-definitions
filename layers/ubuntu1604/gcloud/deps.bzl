...
def deps():
    ...
    if "ubuntu1604_gcloud_debs" not in excludes:
        http_file(
            ...
            downloaded_file_path = DEBS_TARBALL.revision + "_gcloud_debs.tar",
            sha256 = DEBS_TARBALL.sha256,
            urls = [
                "https://storage.googleapis.com/layer-deps/ubuntu1604/gcloud/debs/" + DEBS_TARBALL.revision + "_gcloud_debs.tar",
            ],
        )
